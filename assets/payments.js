...
function checkPaymentReturn() {
  ...
  if (urlParams.get("cancelled") === "1") {
    UIHelpers.showToast("Payment was cancelled", "warning");
    // Clean up URL
  ...
  if (error) {
    UIHelpers.showToast(`Payment failed: ${error}`, "error");
    // Clean up URL
    window.history.replaceState({}, document.title, window.location.pathname);
    return;
  }
...
function showLoading() {
  ...
  if (
    dueDateElement &&
    !dueDateElement.classList.contains("loading-skeleton")
  ) {
...
function updateTotalDisplay(totalDue) {
  const amountElement = document.getElementById("totalAmount");
  if (amountElement) {
    amountElement.textContent = `₱${totalDue
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
  }
}
  ...
  if (dueDateElement) {
    ...
    if (bills.length > 0) {
      const earliestBill = bills.reduce((earliest, bill) => {
        return new Date(bill.due_date) < new Date(earliest.due_date)
          ? bill
          : earliest;
      });
      const formattedDate = new Date(earliestBill.due_date).toLocaleDateString(
        "en-US",
        {
          month: "short",
          day: "numeric",
  ...
  if (statusElement) {
    ...
    if (totalDue === 0) {
      ...
      statusElement.className =
        "bg-green-400 text-green-900 px-3 py-1 rounded-md text-xs font-bold";
    } else {
      const hasOverdue = bills.some((bill) => bill.status === "overdue");
      if (hasOverdue) {
        statusElement.textContent = "Overdue";
        statusElement.className =
          "bg-red-400 text-red-900 px-3 py-1 rounded-md text-xs font-bold";
      } else {
        statusElement.textContent = "Payment Required";
        statusElement.className =
          "bg-amber-400 text-amber-900 px-3 py-1 rounded-md text-xs font-bold";
      }
    }
  }
}
            ...
            <div class="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">${UIHelpers.formatDate(
                          tx.created_at
                        )} • ${tx.transaction_id}</p>
                    </div>
                </div>
                <div class="text-right">
                    <div class="font-bold text-slate-900">${UIHelpers.formatCurrency(
                      tx.amount
                    )}</div>
                    <div class="text-xs text-green-600 font-medium capitalize">${
                      tx.status
                    }</div>
                </div>
            </div>
    ...
    .map((bill) => {
      ...
      const itemsHtml = items
        .map((item) => {
          ...
          const iconColor =
            item.category === "water" ? "text-blue-500" : "text-amber-500";
          ...
          return `
                <div class="flex items-start gap-3 mt-2">
                    ...
                    <div class="font-bold text-slate-900">₱${parseFloat(
                      item.amount
                    ).toFixed(2)}</div>
                </div>
            `;
            ...
            <div class="p-4 hover:bg-slate-50 transition-colors">
                <div class="flex items-start justify-between gap-4 mb-3">
                    <div>
                        ...
                        <p class="text-xs ${statusClass} capitalize mt-1">Status: ${
        bill.status
      }</p>
                        ${statusBadge}
                    </div>
                    <div class="text-right">
                        <div class="font-bold text-slate-900 text-lg">₱${parseFloat(
                          bill.amount
                        ).toFixed(2)}</div>
                        <div class="text-xs text-slate-500">Due: ${formatDate(
                          bill.due_date
                        )}</div>
                    </div>
                </div>
                ${
                  items.length > 0
                    ? `<div class="border-t border-slate-100 pt-3 space-y-2">${itemsHtml}</div>`
                    : ""
                }
            </div>
        `;
...
function renderTransactionHistory() {
  ...
  listContainer.innerHTML = history
    .map((tx) => {
      ...
      const methodDisplay =
        methodNames[tx.payment_method] || tx.payment_method || "Payment";
            ...
            <div class="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">${formatDate(
                          tx.created_at
                        )} • ${tx.reference_number || "N/A"}</p>
                    </div>
                </div>
                <div class="text-right">
                    <div class="font-bold text-slate-900">₱${parseFloat(
...
function attachEventListeners() {
  ...
  if (paypalBtn) {
    paypalBtn.addEventListener("click", async function () {
      await processPayment("paypal"); // PayPal direct integration
    });
  }
  ...
  try {
    ...
    const response = await fetch("/api/payments/process", {
      ...
      body: JSON.stringify({
        ...
        gateway: gateway,
        amount: totalDue,
        return_url: window.location.origin + "/pages/user/payment-success.php",
        cancel_url:
          window.location.origin + "/pages/user/payments.php?cancelled=1",
      }),
    });
    ...
    if (result.success && result.data.payment_url) {
      // Store transaction info for when user returns
