                ...
                <form id="loginForm" class="block" novalidate>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <p id="passwordError" class="hidden text-xs text-red-600 ml-1 mt-1 flex items-center gap-1">
                                <i data-lucide="alert-circle" class="w-3 h-3"></i>
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.errors && (result.errors.email || result.errors.password)) {
                    ...
                    if (result.errors.password) showFieldError('password', result.errors.password);
                } else {
            ...
            if (error.response) {
                ...
                if (status === 401 || status === 422) {
                    ...
                    showFieldError('email', 'Email account not found.');
                } else {
                    showError('Login failed. Please try again.');
                }
            } else if (!navigator.onLine) {
                showFieldError('email', 'No internet connection.');
            } else {
    ...
    if (authToken) {
        ApiClient.get('/auth/me').then(result => {
            if (result.success) {
                ...
                else if (role === 'technician') window.location.href = '/technician-dashboard.php';
